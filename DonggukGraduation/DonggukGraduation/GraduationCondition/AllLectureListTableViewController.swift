...
class AllLectureListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if category == "전공영역" {
            if indexPath.section == 0 {
                ...
                cell1.pointLabel.text = "\(lecture.credit)학점"
    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        if category == "전공영역" {
            ...
            if section == 0 {
                let myCredit = ((myCurri?.majorCredit)! - (myCurri?.majorSpecialty)!)
                let curriCredit = ((departmentCurri?.majorCredit)! - (departmentCurri?.majorSpecialty)!)
                
                return "전공기초 \(String(describing: myCredit)) /  \(String(describing: curriCredit))학점"
            }   
            else if section == 1 {
                return "전공전문 \((myCurri?.majorSpecialty)!) /  \((departmentCurri?.majorSpecialty)!)학점"
            }
        }
        ...
        let lecture = lectures.filter{$0.category == sCategory}
        
        if lecture.count == 0 { return nil }
        
        if section == 1 { return "공통교양 \((myCurri?.generalCommon)!) /  \((departmentCurri?.generalCommon)!)학점" }
        else if section == 2 { return "학문기초 \((myCurri?.generalBasic)!) / \((departmentCurri?.generalBasic)!)학점" }
        else if section == 3 { return "기본소양 \((myCurri?.generalLiteracy)!) / \((departmentCurri?.generalLiteracy)!)학점" }
        else if section == 4 { return "핵심교양 \((myCurri?.generalMain)!) / \((departmentCurri?.generalMain)!)학점" }
        else if section == 5 { return "대학전공기초 \((myCurri?.generalMajorBasic)!) / \((departmentCurri?.generalMajorBasic)!)" }
        else { return "일반교양 \((myCurri?.generalCulture)!)학점" }
    }
